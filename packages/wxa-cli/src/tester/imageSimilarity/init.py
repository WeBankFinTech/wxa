...
cwd_path = os.getcwd()
target_path = os.path.join(cwd_path, '__wxa_e2e_test__')
...
class DiffImages():
    ...
    def FindAndDiffImgs(self, ins):
        ...
        for cur_dir in dirnames:
            ...
            # 拼接路径
            cur_base = os.path.join(target_path , cur_dir, "base_screenshot")
            cur_replay = os.path.join(target_path, ".replay_result", ins, cur_dir, "screenshot")
            # 拿到目录下的图片列表
            ...
            for base_img in base_img_list:
                ...
                if exist_replay > 0:
                    image1 = self.GetImage(img_path = os.path.join(cur_base, base_img))
                    image2 = self.GetImage(img_path = os.path.join(cur_replay, base_img))
                    diff_result = self.dHash.classfiy_dHash(image1=image1, image2=image2, size=(414, 688))
                    print(os.path.join(cur_dir, base_img) + " diff result is: " + str(diff_result))

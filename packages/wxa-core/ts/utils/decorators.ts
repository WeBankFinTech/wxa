...
const methodDescriptorGenerator = (name, fn, placement='prototype')=>{
    return {
...
const classFactory = (name, fn)=>{
    return function(classDescriptor) {
        const {elements} = classDescriptor;
 ...
 * @return {any}
 */
function Time(name: string, ...rest: any[]) {
    const h = (target?, key?, descriptor?)=>{
        const fn = descriptor.value;
        let timer;
